...
static volatile power_wakeup_source_t wakeup_source = POWER_WAKEUP_SOURCE_NONE;
...
bool power_manager_configure_rtc_wakeup(void)
{
	...
	if(HAL_RTCEx_SetWakeUpTimer_IT(power_config.hrtc, power_config.rtc_wakeup_counter, power_config.rtc_wakeup_clock) != HAL_OK)
	{
		return false;
	}
	return true;
...
{
	wakeup_source = POWER_WAKEUP_SOURCE_NONE;
}
power_reset_source_t power_manager_detect_reset_source(RTC_HandleTypeDef *hrtc)
{
	...
	{
		sourse = POWER_RESET_SOURCE_STANDBY_RTC;
	}
	else
	{
...
void HAL_RTCEx_WakeUpTimerEventCallback(RTC_HandleTypeDef *hrtc)
{
	...
	if(hrtc == power_config.hrtc)
	{
		wakeup_source = POWER_WAKEUP_SOURCE_RTC;
	}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	if(GPIO_Pin == power_config.button_gpio_pin)
	{
		wakeup_source = POWER_WAKEUP_SOURCE_BUTTON;
	}
}
